...
# Compute the glistening angle that will be used to comput the glistening zone
def glistening_angle(x):
    """
    x = [km] glistening zone's width
    """
...
# Compute the resolution of the beam
...
# Transform the geocentric coordinates into cartesian
...
def radiation_pattern_partial(pos_sat,p,acc):
    ...
    Parameters
    ----------
    pos_sat : Numpy array with dimensions : N x 3
        Contains the cartesian coordinates of the N antenna's.
    acc : float
        Accuracy of the computation, e.g. : 0.1° or 0.01°.

    Returns
    -------
    F : Numpy array with dimensions : m x 1 
        Radiation Pattern.
    """
    ...
    for i in range(m): #boucle sur les phi
        ...
        #u[i,0,:] = [np.sin(np.radians(phi)) , 0 , np.cos(np.radians(phi))] #unit director vector u pour theta=0°
...
def pos_sat_with_specular_point(Time):
    ...
    if Spiral == 1:
        ...
        if visibility == 1:
            ...
            ax.scatter(specular_point[0], specular_point[1], specular_point[2], color='r')
            print("Specular Point :", specular_point)
            ax.plot([pos_Cubesat[0], specular_point[0]], [pos_Cubesat[1], specular_point[1]], [pos_Cubesat[2], specular_point[2]], linestyle='dashed', color='orangered',label='Cubesats -> Specular Point')
            ax.plot([pos_GNSS[0], specular_point[0]], [pos_GNSS[1], specular_point[1]], [pos_GNSS[2], specular_point[2]], linestyle='dashed', color='orange',label='GNSS -> Specular Point')
...
def pos_sat(Time):
    ...
    if Spiral_shape == 1: 
        ...
        plt.savefig("Spiral 5 Config.svg")
    


    # ------------------ Y/X formation (and straight line) -------------------
    if xy_shape == 1:
        ...
        if Y_shape == 1:
            y0_list = np.linspace(5,50,20) # Pour le Y
            y0_list2 = np.linspace(-5,-50,10)
        elif X_shape == 1:
            y0_list = np.linspace(1,48,15)
            y0_list2 = np.linspace(-1,-48,15) # Pour le X
    ...
    #---------------------- Formation flight for 2 satellites in 2D - left and right -------------------- 
